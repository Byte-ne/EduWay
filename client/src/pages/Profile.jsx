                        ...
                        <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--border-light)' }}>
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                                ...
                                <button type="button" onClick={async () => {
                                    ...
                                    try {
                                        const token = localStorage.getItem('token')
                                        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/users/search?q=${encodeURIComponent(searchQuery)}`, { headers: { 'Authorization': `Bearer ${token}` } })
                                        const data = await res.json()
                                    ...
                                    {searchResults.map((r, idx) => (
                                        <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 8, background: 'var(--white)', borderRadius: 8 }}>
                                            ...
                                            <button type="button" onClick={async () => {
                                                try {
                                                    const token = localStorage.getItem('token')
                                                    await fetch(`${import.meta.env.VITE_API_URL}/api/auth/add-buddy`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ buddyId: r._id || r.id }) })
                                                ...
                                                if (n.type === 'buddy_request') return (
                                                    <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 8, background: 'var(--off-white)', borderRadius: 8 }}>
                                                        ...
                                                        <div style={{ display: 'flex', gap: 8 }}>
                                                            <button type="button" onClick={async () => {
                                                                const token = localStorage.getItem('token')
                                                                await fetch('${import.meta.env.VITE_API_URL}/api/auth/respond-buddy', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ buddyId: n.from._id || n.from.id, action: 'accept' }) })
                                                            ...
                                                            <button type="button" onClick={async () => {
                                                                const token = localStorage.getItem('token')
                                                                await fetch('${import.meta.env.VITE_API_URL}/api/auth/respond-buddy', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ buddyId: n.from._id || n.from.id, action: 'reject' }) })
                                                                setNotifications(prev => prev.filter(x => x !== n))
                                                    ...
                                                    <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 8, background: 'var(--off-white)', borderRadius: 8 }}>
                                                        ...
                                                        <div style={{ display: 'flex', gap: 8 }}>
                                                            <button type="button" onClick={async () => {
                                                                try {
                                                                    const token = localStorage.getItem('token')
                                                                    const res = await fetch(`${import.meta.env.VITE_API_URL}/api/challenges/${n.meta && n.meta.challengeId}/respond`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ accept: true }) })
                                                            ...
                                                            <button type="button" onClick={async () => {
                                                                try {
                                                                    const token = localStorage.getItem('token')
                                                                    await fetch(`${import.meta.env.VITE_API_URL}/api/challenges/${n.meta && n.meta.challengeId}/respond`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ accept: false }) })
                                                                    setNotifications(prev => prev.filter(x => x !== n))
                                                                } catch (e) { }
                                    ...
                                    <div style={{ marginTop: 18, paddingTop: 12, borderTop: '1px dashed var(--border-light)' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: 8 }}>
                                            <button type="button" onClick={async () => {
                                                try {
                                                    const token = localStorage.getItem('token')
                                                    await fetch('${import.meta.env.VITE_API_URL}/api/groups', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ name: groupName, members: selectedMembers }) })
