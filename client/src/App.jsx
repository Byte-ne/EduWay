...
function Navigation() {
    ...
    async function markRead(id) {
        try {
            const token = localStorage.getItem('token')
            await fetch('https://final-hackathon-me4n.onrender.com/api/auth/notifications/mark-read', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ id }) })
            fetchNotifications()
        } catch (e) { }
    }
    ...
    async function markAll() {
        try {
            const token = localStorage.getItem('token')
            await fetch('https://final-hackathon-me4n.onrender.com/api/auth/notifications/mark-all', { method: 'POST', headers: { Authorization: `Bearer ${token}` } })
            fetchNotifications()
        } catch (e) { }
    }
